...
public class UndoManager {
	 ...
	 */
	private List<Undoable> redoStack;

	/**
	 * Collection of undo activities
	 */
	private List<Undoable> undoStack;
	...
	public UndoManager(int newUndoStackSize) {
		...
		undoStack = new ArrayList<Undoable>(maxStackCapacity);
		redoStack = new ArrayList<Undoable>(maxStackCapacity);
	}
	 ...
	 */
	private void removeFirstElementInFullList(List<Undoable> l) {
	...
	private Undoable getLastElement(List<Undoable> l) {
		if (l.size() > 0) {
	...
	protected void clearStack(List<Undoable> clearStack) {
		clearStack.clear();
	}
	...
	public void clearUndos(DrawingView checkDV) {
		Iterator<Undoable> iter = undoStack.iterator();
		while (iter.hasNext()) {
	...
	public void clearRedos(DrawingView checkDV) {
		Iterator<Undoable> iter = redoStack.iterator();
		while (iter.hasNext()) {
