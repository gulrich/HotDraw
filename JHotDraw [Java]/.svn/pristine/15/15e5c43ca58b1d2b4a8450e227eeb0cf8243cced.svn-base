...
class QuadTree implements Serializable {
	...
	private static final long serialVersionUID = -2989788971735637894L;
	
	private Rectangle2D  _absoluteBoundingRectangle2D = new Rectangle2D.Double();
	private int          _nMaxTreeDepth;
	private Hashtable<Figure, Rectangle2D>    _theHashtable = new Hashtable<Figure, Rectangle2D>();
	private Hashtable<Figure, Rectangle2D>   _outsideHashtable = new Hashtable<Figure, Rectangle2D>();
	...
	public void add(Figure anObject, Rectangle2D absoluteBoundingRectangle2D) {
		if (_nMaxTreeDepth == 1) {
...
*/
	public FigureEnumerator getAllWithin(Rectangle2D r) {
		List<Figure> l = new ArrayList<Figure>();
		for (Iterator<Figure> ii = _outsideHashtable.keySet().iterator(); ii.hasNext(); ) {
			Figure anObject = ii.next();
		...
		if (_absoluteBoundingRectangle2D.intersects(r)) {
			for(Iterator<Figure> i = _theHashtable.keySet().iterator(); i.hasNext(); ) {
				Figure anObject = i.next();
				Rectangle2D itsAbsoluteBoundingRectangle2D = (Rectangle2D)
			...
			if (_nMaxTreeDepth > 1) {
				return new FigureEnumerator(l)
					.addAll(_nwQuadTree.getAllWithin(r))
					.addAll(_neQuadTree.getAllWithin(r))
					.addAll(_swQuadTree.getAllWithin(r))
					.addAll(_seQuadTree.getAllWithin(r));
			}
		}
