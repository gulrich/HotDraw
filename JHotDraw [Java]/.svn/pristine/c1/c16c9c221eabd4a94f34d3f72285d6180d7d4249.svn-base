...
public class StandardDrawingView
	 ...
	 */
	private transient List<FigureSelectionListener> fSelectionListeners;
	 ...
	 */
	transient private List<Figure> fSelection;

	/**
	 * The shown selection handles.
	 */
	transient private List<Handle> fSelectionHandles;
	 ...
	 */
	private List<Painter> fBackgrounds;
	 ...
	 */
	private List<Painter> fForegrounds;
	...
	public StandardDrawingView(DrawingEditor editor, int width, int height) {
		...
		fSelectionListeners = new ArrayList<FigureSelectionListener>();
		addFigureSelectionListener(editor());
		setLastClick(new Point(0, 0));
		fConstrainer = null;
		fSelection = new ArrayList<Figure>();
	...
	public FigureEnumeration insertFigures(FigureEnumeration fe, int dx, int dy, boolean bCheck) {
		...
		List<Figure> vCF = new ArrayList<Figure>(10);
	...
	public FigureEnumeration getConnectionFigures(Figure inFigure) {
		...
		List<Figure> result = new ArrayList<Figure>(5);
	...
	public FigureEnumeration selectionZOrdered() {
		List<Figure> result = new ArrayList<Figure>(fSelection);
	 ...
	 */
	public void addToSelectionAll(Collection<Figure> figures) {
	...
	protected HandleEnumeration selectionHandles() {
		if (fSelectionHandles == null) {
			fSelectionHandles = new ArrayList<Handle>();
			FigureEnumeration fe = selection();
			while (fe.hasNextFigure()) {
				Figure figure = fe.nextFigure();
	...
	public synchronized void checkDamage() {
		Iterator<DrawingChangeListener> each = drawing().drawingChangeListeners();
		while (each.hasNext()) {
			Object l = each.next();
   ...
   public void draw(Graphics g, FigureEnumeration fe) {
	...
	protected void drawPainters(Graphics g, List<Painter> v) {
	...
	public void addBackground(Painter painter)  {
		if (fBackgrounds == null) {
			fBackgrounds = new ArrayList<Painter>(3);
		}
    ...
    protected List<Painter> getBackgrounds() {
        return fBackgrounds;
    }
	...
	public void addForeground(Painter painter)  {
		if (fForegrounds == null) {
			fForegrounds = new ArrayList<Painter>(3);
		}
		fForegrounds.add(painter);
		repaint();
	}

    protected List<Painter> getForegrounds() {
        return fForegrounds;
    }
	...
	private void readObject(ObjectInputStream s)
		...
		fSelection = new ArrayList<Figure>(); // could use lazy initialization instead
